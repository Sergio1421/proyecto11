...
// - Universidad Tecnológica de Pereira
// - Programa de Ingeniería de Sistemas y Computación
// - Calcula series de taylor con coseno hiperbolico de x
// Salvedad: Para valores fuera de este rango, no garantizamos los resultados.
...
const factorial = (x) => {
  ...
  const main = () => {
    let t, x, s = 1;
    console.log("Este programa imprime un resultado para cosh(x) utilizando una serie de Taylor");
    x = parseFloat(prompt("Ingrese un valor para x: "));
    t = parseInt(prompt("Ingrese el número de términos de la serie: "));
    for (let i = 1; i < t; i++) {
      s = s + (Math.pow(x, 2*i) / factorial(2*i));
    }
